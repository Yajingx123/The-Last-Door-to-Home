...
public class UIManager : MonoBehaviour
{
    ...
    [Tooltip("Optional direct reference to the pause screen object")]
    ...
    public void TogglePause()
    {
        ...
        {
            if (pages != null && pages.Count > pausePageIndex && pages[pausePageIndex] != null)
            {
    ...
    private void Start()
    {
        ...
        if (pauseScreen != null)
            pauseScreen.SetActive(false);
        SetUpPauseScreenContinueButton();
        UpdateUI();
    }
    ...
    private void CheckPauseInput()
    {
        ...
        bool escTriggered = Keyboard.current != null && Keyboard.current.escapeKey.wasPressedThisFrame;

        if (pauseTriggered || escTriggered)
        {
            TogglePause();
        }
    }
    ...
    /// <summary>
    /// Ensures the pause screen continue button has text and resumes game on click.
    /// </summary>
    private void SetUpPauseScreenContinueButton()
    {
        GameObject continueBtnObject = GameObject.Find("Pause Screen/ContinueBtn");
        if (continueBtnObject == null)
            return;

        Button continueButton = continueBtnObject.GetComponent<Button>();
        if (continueButton != null)
        {
            continueButton.onClick.RemoveListener(TogglePause);
            continueButton.onClick.AddListener(TogglePause);
        }

        Transform labelTransform = continueBtnObject.transform.Find("Label");
        TextMeshPro label;

        if (labelTransform == null)
        {
            GameObject labelObject = new GameObject("Label");
            labelObject.transform.SetParent(continueBtnObject.transform, false);
            label = labelObject.AddComponent<TextMeshPro>();
            label.alignment = TextAlignmentOptions.Center;
            label.fontSize = 2.2f;
            label.color = Color.black;
            label.rectTransform.sizeDelta = new Vector2(6f, 2f);
            label.rectTransform.localPosition = new Vector3(0f, 0f, -0.05f);
        }
        else
        {
